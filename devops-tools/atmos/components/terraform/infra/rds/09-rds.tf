...
resource "aws_db_instance" "db_instance_1" {
  ...
  username                            = local.rds_credentials_1["username"]
  password                            = local.rds_credentials_1["password"]
  ...
  db_subnet_group_name = aws_db_subnet_group.subnet_group_1.name

  copy_tags_to_snapshot  = var.rds_copy_tags_to_snapshot
  deletion_protection    = var.rds_deletion_protection
  vpc_security_group_ids = [aws_security_group.rds_security_group_1.id]

  depends_on = [aws_db_subnet_group.subnet_group_1]
