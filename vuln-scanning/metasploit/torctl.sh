...
start_service() {
    SERVICE=tor
    if [[ $(service is-active $SERVICE) != "active" ]]; then
        warn "$SERVICE is not started"
        info "starting $SERVICE service"
        service $SERVICE start || err "unable to start $SERVICE service"
        msg "started $SERVICE service"
    else
        warn "$SERVICE is running"
        info "reloading $SERVICE service"
        service $SERVICE reload || err "unable to reload $SERVICE service"
...
stop_service() {
    ...
    if [[ $(service is-active $SERVICE) == "active" ]]; then
        ...
        service $SERVICE stop || err "unable to stop $SERVICE service"
...
status() {
    TORSTATUS=$(service is-active tor)

    AUTOWIPESTATUS=$(service is-enabled torctl-autowipe)

    AUTOSTARTSTATUS=$(service is-enabled torctl-autostart)
...
autowipe() {
    ...
    service enable torctl-autowipe &>"/dev/null"
    msg "enabled torctl-autowipe"
}

autostart() {
    warn "enabling torctl-autostart"
    service enable torctl-autostart &>"/dev/null"
