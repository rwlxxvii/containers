<# 
ScriptName: tenable_exporter.ps1
PSVersion:  5.1
...
$AccessKey = Read-Host "Enter Access Key" -AsSecureString
$SecretKey = Read-Host "Enter Secret Key" -AsSecureString
$akey = [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($AccessKeyResponse))
$skey = [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($SecretKeyResponse))
...
$UserNameBody = convertto-json (New-Object PSObject -Property @{accesskey = $akey; secretkey = $skey})
...
$Header = @{'x-apikey'=@{'accesskey'=$akey;'secretkey'=$skey}}
