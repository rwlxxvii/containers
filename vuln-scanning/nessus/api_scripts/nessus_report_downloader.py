...
#generate keys for a user account in nessus first and place here before running script
access_key="   "
secret_key="   "
...
def downloadNessusReport(base_url, headers, scan_id_list, json_user_data):
    for scan_id in scan_id_list:
        ...
        resp = sendPostRequest(url, json_data=json_user_data,headers=headers)
        ...
        resp2 = sendGetRequest(url,headers=headers)
        ...
        url= base_url + "/scans/{0}/export/{0}/download".format(str(file_token["token"]))
        if json.loads(resp2.text)["status"] == "ready":
            printMessage("Download link is available now", 1)
            resp3 = sendGetRequest(url,headers=headers)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="A python script for automating the report download from nessus server",
                                     epilog="Report bugs at nowhere",
                                     prog='python nessus_report_downloader.py', usage='%(prog)s -i <127.0.0.1> [OPTIONS]... ')
    ...
    if checkStatus(resp, "Connected to nessus server", "Unable to connect to server at " + str(ip)):
            ...
            resp = sendGetRequest(base_url + "/scans", headers={'X-ApiKeys': "accessKey={}; secretKey={}".format(access_key, secret_key)})
            if checkStatus(resp, "Fetching scan reports\n", "Unable to fetch nessus scan"):
                ...
                # Download the report if scan_id or folder_id supplied via -s or -d option
                    ...
                    for report_format in format_specification:
                        downloadNessusReport(base_url, headers, scan_id_list, json_user_data=report_format)

            # Logout
            resp = sendDeleteRequest(base_url+"/session",headers=headers)
