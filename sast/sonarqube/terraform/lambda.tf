resource "aws_lambda_function" "process_scans" {
  ...
  vpc_config {
    subnet_ids         = [aws_subnet.dev-sonarqube-subnet.id]
    security_group_ids = [aws_security_group.sonarqube-sg.id]
  }
...
#  name                = "sonarqube-24-hours"
#  description         = "Fire sonarqube scan every 24 hours"
